...
class Two extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const a = prevBoard.find(arr => arr.find(score => score === this.state.winningScore));
    const b = this.state.board.find(arr => arr.find(score => score === this.state.winningScore));
    if (!a && b) {
      this.setState({ answer: this.props.question });
    }
  }
  getButtons() {
    ...
    case 1:
      return <div className="buttons">
        <div className="button" style={{ marginTop: 30 }} onClick={() => { this.move('left') }}>&larr;</div>
        <div>
          <div className="button" onClick={() => { this.move('up') }}>&uarr;</div>
          <div className="button" onClick={() => { this.move('down') }}>&darr;</div>
        </div>
        <div className="button" style={{ marginTop: 30 }} onClick={() => { this.move('right') }}>	&rarr;</div>
      </div>;
    case 2:
      return <div className="buttons">
        <div className="button" style={{ marginTop: 30 }} onClick={() => { this.move('right') }}>	&rarr;</div>
    ...
    case 3:
      return <div className="buttons">
        <div className="button" style={{ marginTop: 30 }} onClick={() => { this.move('up') }}>&larr;</div>
        <div>
          <div className="button" onClick={() => { this.move('right') }}>&uarr;</div>
          <div className="button" onClick={() => { this.move('left') }}>&darr;</div>
        </div>
        <div className="button" style={{ marginTop: 30 }} onClick={() => { this.move('down') }}>	&rarr;</div>
      </div>;
    }
  }
  getResetButton() {
    return <div className="button" style={{ marginBottom: 10 }} onClick={this.initBoard.bind(this)}>&#x21BB;</div>;
  }
  render() {
    if (this.state.message) return <div>
      <p style={{ marginTop: 5, marginBottom: 10 }}>{this.state.message}</p>
      {this.getResetButton()}
    </div>
    if (this.state.answer) return <div>{this.state.answer}</div>;
    return <div style={{ marginTop: -20 }}>
      <p style={{ margin: 5 }}>{this.state.winningScore}</p>
      {/* {this.getResetButton()} */}
